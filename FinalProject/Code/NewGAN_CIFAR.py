...
def plot_gen(n_ex=16,dim=(4,4), figsize=(10,10) ):
    noise = np.random.uniform(0,1,size=[n_ex,3072])
    noise = noise.reshape(n_ex, 32, 32, 3)
...
noise_gen = np.random.uniform(0,1,size=[XT.shape[0],3072])
noise_gen = noise_gen.reshape(ntrain, 32, 32, 3)
...
def train_for_n(nb_epoch=1000, plt_frq=25,BATCH_SIZE=32):
    ...
    for e in tqdm(range(nb_epoch)):  
        ...
        noise_gen = np.random.uniform(0,1,size=[BATCH_SIZE,3072])
        noise_gen = noise_gen.reshape(BATCH_SIZE, 32, 32, 3)
        generated_images = generator.predict(noise_gen)
        ...
        noise_tr = np.random.uniform(0,1,size=[BATCH_SIZE,3072])
        noise_tr = noise_tr.reshape(BATCH_SIZE, 32, 32, 3)
